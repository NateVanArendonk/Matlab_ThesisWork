...
out_file = 'SWAN_10m_SGALUT_offshore_extract';
...
inp = inpolygon(lon,lat,M.Lon,M.Lat); % Find what part of the transect is in the Basin
inds = find(inp == 1); % Find the basin points
in2 = vertcat(in2,inds); % Add them to the list
clear kml_load
...
specialCase = length(E.x)-3:length(E.x);
for ii = 3:length(E.z)
    count = 0;
