...
% NOTE THIS FUCKER TAKES 5 HOURS TO RUN
clearvars
...
for kk = 1:length(keys) % Loop through the basin masks and grab subset of points to make hindcast of
    ...
    for ff = 1:length(L.x)
        ...
        if ~exist(['E:\Abbas\PS_COSMOS\Thesis_Modeling\LUT\Hindcast_Output\' outName])
            ...
            if ~isnan(temp(2,1))
                if slr
                    hs_ts(1,:) = interp3(X,Y,Z,temp,twl+slr,speed,wnddir,'linear')';
                else
                ...
                temp(nan_inds) = 0;
                if slr
                    tp_ts(1,:) = interp3(X,Y,Z,temp,twl+slr,speed,wnddir,'linear')';
                else
                ...
                temp(nan_inds) = 0;
                if slr
                    tm_ts(1,:) = interp3(X,Y,Z,temp,twl+slr,speed,wnddir,'linear')';
                else
                ...
                temp(nan_inds) = 0;
                if slr
                    dir_ts(1,:) = interp3(X,Y,Z,temp,twl+slr,speed,wnddir,'linear')';
                else
