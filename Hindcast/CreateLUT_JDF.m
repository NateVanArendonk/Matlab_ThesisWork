...
out_file = 'SWAN_10m_JDFLUT_offshore_extract';
...
% Find which points are NOT within Southern Domain
in = [];
...
M = kml2struct(kml_mask);
in = [];
inp = inpolygon(K.Lon,K.Lat,M.Lon,M.Lat); % Find what part of the transect is in the Basin
inds = find(inp == 1); % Find the basin points
in = vertcat(in,inds); % Add them to the list
clear kml_load
