...
kml_fol = 'E:\Abbas\PS_COSMOS\Thesis_Modeling\KML\PugetSoundShorline\SouthPSIslandShorelines\'; % Location of shoreline KML to use
kml_list = dir('E:\Abbas\PS_COSMOS\Thesis_Modeling\KML\PugetSoundShorline\SouthPSIslandShorelines\*.kml'); % List of all island alongshore transects
...
for kk = 1:length(kml_list)
    ...
    if ~exist(['Offshore_LUT_Extracted\' out_file])
        ...
        ss = 3;
        dd = 10;
        fname = sprintf('SpatialPS_s%02d_d%d_t%d.mat',speed(ss)*10,direc(dd),tide(tt)*10); % Loads in a low tide level
