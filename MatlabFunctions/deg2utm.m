...
for i=1:n1
   ...
   x(i)=xx;
   y(i)=yy;
   % Logic Added in the situation that a NaN is present for any of the
   % points except the first.  Will write logic for that if that ever
   % arises.  NV - 9/21/18 - Earth Wind and Fire ;) 
   if isnan(Huso) && i ~= 1
       utmzone(i,:) = utmzone(i-1,:);
   else
