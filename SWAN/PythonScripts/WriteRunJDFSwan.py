...
for ii in range(0,len(forcing_list)):
    ...
    unique_name = "SpatialJDF_s%d_d%d_t%d.mat" % (current_forcings[0]*10,current_forcings[1],current_forcings[2]*10)
    with cd("RES1"):
...
$ Wind input (no variation)"""
                ...
                f.writelines("\nWIND %.1f %.1f DRAG WU" % (current_forcings[0], current_forcings[1]))
