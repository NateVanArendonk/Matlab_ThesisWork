...
M = load('SentryShoal_WRFwaveout.mat');

% Load in Buoy Data
O = load('../NNRP/QAQC_SentryWaves.mat');
% Buoy data is recorded on minute 9 but since that value represents the
% hour I will just remake the time vector to be on the top of the hour 
% And interp on to that 
time = datenum(1992,10,20,12,00,00):1/24:datenum(2017,6,20,22,00,00);
O.hs = interp1(O.time,O.Hs_2,time);
O.tp = interp1(O.time,O.Tp_2,time);
O.time_i = time;
%%
...
modelInds = modelStart:length(M.time);
for ii = 1:length(M)
...
% Subsample obs now 
modelEnd = find(O.time_i == M.time(end));
modelEnd = findnearest(M.time(end),O.time_i);
inds = 1:modelEnd;
% O.time = O.time_i(inds);
% O.quality_code = O.quality_code(inds);
O.hs = O.hs(inds);
O.tp = O.tp(inds);
O.time_i = O.time_i(inds);
