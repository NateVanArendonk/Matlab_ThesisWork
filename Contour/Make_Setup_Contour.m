...
%%
path = 'E:\Abbas\WCRP\Tacoma\Tier3\XBeach\OB_RW\10yrScenario\Contemporary\'; % Path to the model runs
...
for ii = 1:length(X)
    ...
    s_thresh = 0.05; % 2 cm depth cutoff for setup
...
for ii = 1:length(X)
    %     X(ii).z = fliplr(X(ii).z);
    
    ds = .01;
    % Create transect
    [t_x, t_y] = createTransect(X(ii).x(end),X(ii).y(end),X(ii).x(1),X(ii).y(1),ds);
    
    % Make Finely spaced vectors of s and z
    t_s = sqrt(t_x.^2+t_y.^2);
    t_s = t_s - min(t_s);
    t_z = interp1(X(ii).s,X(ii).z,t_s);
    t_z = fliplr(t_z); % Flip so first point is off shore - inline with x,y order
    X(ii).t_s = t_s;
    X(ii).t_x = t_x;
    X(ii).t_z = t_z;
    X(ii).t_y = t_y;
    
    % Find Intersection of Profile and R2 elevation
    rline = repmat(X(ii).maxSUP,1,2);% Take off 3cm to be sure we don't exceed any berms for showing R2 extent
    xline = [0 length(t_s)];
    P = InterX([t_s;t_z],[xline;rline]);
    beginDistance = t_s(1) - P(1,:);% distance to s value of intial point = most off shore point
    [~,I] = min(beginDistance);
    P = P(:,I);
    
    % Now translate it back to the X,Y position
    ind = findnearest(P(1),t_s);
    RX(ii) = t_x(ind);
    RY(ii) = t_y(ind);
    RS(ii) = t_s(ind);
    RZ(ii) = t_z(ind);
end
%% Not sure why i do this but I think it was for Eric a few weeks ago, will keep for now 
for ii = 1:length(X)
    S(ii).name = X(ii).name;
    S(ii).x = X(ii).x;
    S(ii).y = X(ii).y;
    S(ii).s = X(ii).s;
    S(ii).z = X(ii).z;
    S(ii).swl = X(ii).zs(1,1);
    S(ii).supVal = X(ii).maxSUP;
    S(ii).sup = X(ii).s_up;
end
...
for ii = 2:length(X)
    ...
    if cc > 1
        ...
        for zz = 1:length(px)
            dist = sqrt((px(zz) - a(end)).^2 + (py(zz) - b(end)).^2); 
            dmin = min(dist);
        ...
        for pp = 1:length(px)
            ...
            indsIn = inpolygon(tx1(temp_inds),ty1(temp_inds),[x2(1),x2(end),x1(end),x1(1)],[y2(1),y2(end),y1(end),y1(1)]);
    ...
    if cc > 1
        ...
        elseif numel(px) > 1
            ...
            for pp = 1:length(px)
                dist = sqrt((px(pp)-tx1).^2 + (py(pp)-ty1).^2);
                [~,contourInd(pp)] = min(dist);
                temp_inds = contourInd(pp)-500:contourInd(pp)+500;
...
kmlwriteline('setup_SLR130.kml',lat,lon);
